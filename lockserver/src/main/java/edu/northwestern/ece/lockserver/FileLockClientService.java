...
public final class FileLockClientService implements MessageListener {
    ...
    public Message accept(Message m) {
        ...
        if (msgType == FILE_LOCK_CLIENT_SERVICE_CLIENT_MESSAGE) {
            ...
            if (action == null) {
                throw new FileLockServerException("Null action");
            }

            try {
                switch (action.toLowerCase()) {
                    case "acquire":
                        return acquireLocks(m);
                    case "acquiremore":
                        return acquireMoreLocks(m);
                    case "release":
                    case "releaseblock":
                        return releaseLocks(m);
                    case "acquireblock":
                        return acquireBlockLocks(m);
                    case "acquiremoreblock":
                        return acquireMoreBlockLocks(m);
                    default:
                        throw new FileLockServerException("Unknown action: " + action);
                }
            } catch (IOException e) {
                throw new FileLockServerException(e);
            }
        } else if (msgType == FILE_LOCK_CLIENT_SERVICE_SERVER_MESSAGE) {
            throw new FileLockServerException("cannot support this type of message yet");
        } else {
            throw new FileLockServerException("Unknown message type: " + msgType);
        }
    }
    ...
    protected Message acquireMoreLocks(Message m) throws IOException {
        ...
        List<FileLock> lockList = new ArrayList<>();
     ...
     * @param locks    List of FileLock elements
     ...
     * @param fileName name of file to lock
     * @param locks    List of FileLock elements
     * @param lockID   the ID for these locks (acquired from previous call to
     *                 getLocks()
     * @return 2-element list: lock ID and number of locks granted
     */
    ...
    private static Message getBlockLocks(String fileName, List blocks, int lockID) throws IOException {
        MessageClient client = new MessageClient(FILE_LOCK_SERVER_SERVICE_NAME,
                FileLockServerService.FILE_LOCK_SERVER_SERVICE_PORT);
        ...
        for (int i = 0; blockIter.hasNext(); ++i) {
            ...
            m.setLongArrayParamValue("blocks", i, b);
        }
